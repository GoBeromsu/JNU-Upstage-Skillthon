...
def parse_parts_html(html):
    ...
    # 부품 데이터 추출 (&ptno= 가 HTML 상에서 &amp;ptno= 로 렌더링되는 문제 해결)
    parts = re.findall(
...
def get_models(page, maker="H", vtyp="P"):
    """차량 모델 목록 조회"""
    resp = page.evaluate(f"""
        async () => {{
            const resp = await fetch('/member/join/getCarModel.ajax', {{
                method: 'POST',
    ...
    try:
        ...
        model_list = data.get("model", {}).get("list", [])
        return [{"id": str(item["wpcCatSeq"]), "name": item["cpnm"]} for item in model_list]
    except Exception as e:
        print(f"모델 JSON 파싱 실패: {e}", file=sys.stderr)
        return []
...
if __name__ == "__main__":
    ...
    with sync_playwright() as p:
        ...
        if args.list_models:
            models = get_models(page, args.maker, args.vtyp)
